...
teto=strlen(buscapalavra);
    while (!(feof(Searcher)))
    {
        ...
        while (count <strlen(search))
        {
            ...
            for(i=chao;i<=teto;i++)
            {
                if (buscapalavra[j]==search[i]) existe=true; else { existe = false;  break;
                }
                j++;
            }
            chao++;
            teto++;
            if (existe==true){printf("%s\n",search);numberofwords++; break; }
        count++;
        }
        }
    }
...
int Comand()
{
    ...
    if (!(strcmp(Mensagem,"/nick")))    { printf("Digite o seu novo nick :"); gets(Nick); ReturnValue=1; }
    if (!(strcmp(Mensagem,"/help")))    { system("cls"); printf ("Digite /exit para sair.\nDigite /nick para mudar o seu nick.\nDigite /cls para limpar a tela de escrita.\nDigite /busca para buscar algo na conversa.\n"); ReturnValue=1; }
    if (!(strcmp(Mensagem,"/cls")))     {system ("cls"); ReturnValue=1; }
    if (!(strcmp(Mensagem,"/busca")))   {system ("cls"); Busca();ReturnValue=1; }
       return(ReturnValue);
}
